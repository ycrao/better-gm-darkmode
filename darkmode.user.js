// ==UserScript==
// @name              更好的夜间模式助手
// @namespace         https://github.com/ycrao/better-gm-darkmode
// @version           3.0.1
// @description       实现任意网站的夜间模式，支持网站白名单
// @author            ycrao && YouXiaoHou
// @license           MIT
// @homepage          https://github.com/ycrao/better-gm-darkmode
// @supportURL        https://github.com/ycrao/better-gm-darkmode
// @updateURL         https://raw.githubusercontent.com/ycrao/better-gm-darkmode/refs/heads/main/darkmode.user.js
// @downloadURL       https://raw.githubusercontent.com/ycrao/better-gm-darkmode/refs/heads/main/darkmode.user.js
// @match             *://*/*
...
;(function () {
    ...
    let main = {
        ...
        registerMenuCommand() {
            if (this.isTopWindow()) {
                let whitelist = util.getValue('whitelist');
                let host = location.host;
                if (whitelist.includes(host)) {
                    GM_registerMenuCommand('💡 当前网站：✔️ 已启用', () => {
                        let index = whitelist.indexOf(host);
                        whitelist.splice(index, 1);
                        util.setValue('whitelist', whitelist);
                        history.go(0);
                    });
                } else {
                    GM_registerMenuCommand('💡 当前网站：❌ 已禁用', () => {
                        whitelist.push(host);
                        util.setValue('whitelist', Array.from(new Set(whitelist)));
                        history.go(0);
                    });
                }
                ...
                GM_registerMenuCommand('⚙️ 设置', () => {
                    ...
                    let whitelistStr = util.getValue('whitelist').join('\n');
                    ...
                    Swal.fire({
                        ...
                        footer: '<div style="text-align: center;font-size: 1em;">点击查看 <a href="https://github.com/ycrao/better-gm-darkmode" target="_blank">使用说明</a>，助手免费开源，modified by <a href="https://github.com/ycrao">ycrao</a></div>',
                        customClass: {
                    ...
                    document.getElementById('S-Dark-Whitelist').addEventListener('change', (e) => {
                        util.setValue('whitelist', Array.from(new Set(e.currentTarget.value.split('\n').filter(Boolean))));
                    });
                });
            }
        },
        ...
        isInWhitelist() {
            return util.getValue('whitelist').includes(location.host);
        },
